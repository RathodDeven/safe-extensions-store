...
export const RelayPlugin: FunctionComponent<{}> = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const maxFee = await getMaxFeePerToken(
  ...
  const updateMaxFee = useCallback(
    async (feeTokenInfo: TokenInfo, maxFeeInput: string) => {
      console.log("UPDATE");
      const targetMaxFee = ethers.utils.parseUnits(
        maxFeeInput,
        feeTokenInfo.decimals
      );
      await updateMaxFeePerToken(
        feeTokenInfo.address,
        BigInt(Number(targetMaxFee))
      );
    },
    []
  );
