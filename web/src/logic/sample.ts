...
const SAMPLE_PLUGIN_ADDRESS = getAddress(
  "0x60D996eF6F281eD4f562537fab84CFE192FF0206"
);
...
export const isKnownSamplePlugin = (
  ...
  ethers.BigNumber.from(chainId).eq(
    ethers.BigNumber.from(SAMPLE_PLUGIN_CHAIN_ID)
...
export const getTokenInfo = async (address: string): Promise<TokenInfo> => {
  if (address === NATIVE_TOKEN || address === ZERO_ADDRESS)
    return {
      address,
      symbol: "ETH",
      decimals: BigInt(18),
